...
function paginate_buttons(pages) {
    ...
    if (state.page != 1) {
        pagination_nav.innerHTML = `<a value="${state.page-1}" class="paginate_button" data-lang="paginate_1">Previous</a>` + pagination_nav.innerHTML;
    } else {
        pagination_nav.innerHTML = `<a value="${state.page-1}" style="color:#aaa;pointer-events:none;border:0;" class="paginate_button" data-lang="paginate_1">Previous</a>` + pagination_nav.innerHTML;
    }

    if (state.page != pages) {
        pagination_nav.innerHTML += `<a value="${state.page+1}" class="paginate_button" data-lang="paginate_2">Next</a>`;
    } else {
        pagination_nav.innerHTML += `<a value="${state.page+1}" style="color:#aaa;pointer-events:none;border:0;" class="paginate_button" data-lang="paginate_2">Next</a>`;
    }
...
function build_html_data() {
    ...
    try {
        ...
        pagination_entries.innerHTML = `<span data-lang="entries_1">Showing</span> ${data.trimStart+1} <span data-lang="entries_2">to</span> ${data.gamesData[data.gamesData.length-1].rank} <span data-lang="entries_3">of</span> ${state.gamesData.length}`;
    } finally {
...
async function after_submit_id() {
    ...
    const FDSerialized = Object.fromEntries(FD);
    
    if (!FDSerialized.steamId || FDSerialized.steamId === '') return;
    ...
    if (data.success === false && data.error_code === 1) {
        ...
        games_l_title_el.textContent = localStorage.getItem("language") === "ar" ? 'لم يتم العثور على حساب الملف الشخصي' : data.message;
    } else if (data.success === false && data.error_code === 2) {
        games_l_title_el.style.color = 'red';
        games_l_title_el.textContent = localStorage.getItem("language") === "ar" ? 'حساب الملف الشخصي خاص' : data.message;
    } else if (data.success === false && data.error_code === 3) {
        games_l_title_el.style.color = 'red';
        games_l_title_el.textContent = localStorage.getItem("language") === "ar" ? 'خطأ في إدخال البيانات' : data.message;
        setTimeout(() => {
        ...
        localStorage.setItem('userSearchId', FDSerialized.steamId);
        ...
        games_l_title_el.textContent = localStorage.getItem("language") === "ar" ? 'لعبة تم العثور عليها' : 'Game found';
        ...
        history.push(FDSerialized.steamId.toLowerCase());
        ...
        all_time.innerHTML = `${totalPlayTime.toFixed(2)} <span data-lang="hours">hours</span>`;
        ...
        // save id to database
        saveSearchValue(FDSerialized.steamId);
...
clear_button.addEventListener('click', () => {
    ...
    if (!document.querySelector('.skeleton')) {
        games_container.innerHTML = '<div class="d skeleton"></div><div class="d skeleton"></div><div class="d skeleton"></div><div class="d skeleton"></div><div class="d skeleton"></div><div class="d skeleton"></div><div class="d skeleton"></div><div class="d skeleton"></div><div class="d skeleton"></div>';
    }
...
apps_search.addEventListener("input", (event) => {
    ...
    if (input !== '' && state.gamesData.length) {
        const dataFiltered = state.gamesData.filter(v => {
            return v.name.toLowerCase().includes(input) || JSON.stringify(v.appid).includes(input);
        });
    ...
    save_as_buttons.forEach((button) => {
        button.addEventListener('click', (e) => {
            ...
            if (value === 'excel') {
                ...
                a.href = URL.createObjectURL(new Blob([JSON.stringify(state.gamesData, null, 2)], {
                    type: "text/plain"
                }));
...
function startAddingFriends() {
    user_list_btn.addEventListener('click', () => user_list.classList.toggle('active'));
    add_user();
    ...
    friends_search.addEventListener('keyup', () => {
        ...
        arr = state.friendsList.filter(data => {
            ...
            let isSelected = data == user_list_btn.firstElementChild.innerText ? "selected" : "";
            let visibility = data.communityvisibilitystate == '3' ? '<p style="color:lightgreen">public</p>' : '<p style="color:orangered">private</p>';
        ...
        users_options.innerHTML = arr ? arr : `<p style="margin-top: 10px; color: #ccc">Oops! user not found</p>`;
        usersLazyLoading();
    });
}
